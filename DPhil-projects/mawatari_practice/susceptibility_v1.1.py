...
Version 1.1 updates
- Added the ability to automatically write the Tc values to an excel file
- SKIPPED - trying to calc error in Tc but too involved and seems small anyway
...
# Plotting the susceptibility and recording Birr
    ...
    if save:
        ...
        plt.savefig(os.path.join(save_path, f'Raw_susceptibility_{sample_name}.png'))
        plt.close()
    else:
        plt.show()
...
def append_tc_to_file(sample, tc):
    tc_file = pd.read_excel('tc.xlsx', engine='openpyxl', index_col=False)
    
# Check if the sample is new
    if sample not in tc_file['Sample'].values:
        # Create a new DataFrame for the new sample
        new_row = pd.DataFrame({'Sample': [sample], 'Tc': [tc]})
        
        # Append the new DataFrame to the existing DataFrame
        tc_file = pd.concat([tc_file, new_row], ignore_index=True)
        
        # Write DataFrame back to Excel file
        tc_file.to_excel('tc.xlsx', index=False)
...
for sample in sample_names:
    ...
    tc = find_tc_interpolation(data, ix, sample_name=sample, show_plot = show_plot)
    append_tc_to_file(sample, tc)
    plot_susceptibility(data, ix, sample_name=sample, save_path = save_path, tc = tc, save=save) # plots the susceptibility data for the sample
